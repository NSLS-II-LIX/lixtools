...
def perform_cellulose_amporh_separation(args):
    """ qphi_s: background-subtracted I(q,phi) map
        qphi_c: I(q,phi) map for the crystalline data
        xcor,ycor: q and phi coordinates
        BB: basis matrix for NNLS 
        cell_peak_pos: index in q for the cellulose peak
    """

    (sn, qphi_s, qphi_c, xcor, ycor, BB, cell_peak_pos, amorph_range) = args
    
    qidx = (xcor>=cell_peak_pos[0])&(xcor<=cell_peak_pos[1])
    qidx1 = (xcor>=amorph_range[0])&(xcor<=amorph_range[1])
    xcor1 = xcor[qidx1]
    phi_idx = ((ycor>=-90)&(ycor<=90))
    shp_phi = len(ycor[phi_idx])
    prof_phi = np.nansum(qphi_s[:,:,qidx], axis=-1)
    cel = np.zeros_like(qphi_s)
    amor = np.zeros([prof_phi.shape[0], len(xcor1)])

    for k in range(prof_phi.shape[0]):
        if k%100==0:
            print(f"seperating {sn},#{k}     \r", end="")
        vv = np.hstack([prof_phi[k,phi_idx], np.zeros(BB.shape[-1])])
        b,e = nnls(BB, vv)
        cel0 = np.nansum([qphi_c[pw+i-n:pw+i-n+shp_phi,qidx1]*b[i] for i in range(2*n+1)], axis=0)
        sc0 = np.sum(cel0[:,((xcor1>=cell_peak_pos[0])&(xcor1<=cell_peak_pos[1]))])
        if sc0>0:
            sc1 = (np.sum(qphi_s[k, pw:pw+shp_phi, qidx]))/sc0
            amor0 = qphi_s[k, pw:pw+shp_phi, :]
            amor1 = np.nanmedian(amor0[:,qidx1]-cel0*sc1, axis=0)
            amor1[amor1<0] = 0
            amor1 = convolve(amor1, [0.05, 0.1, 0.2, 0.5, 0.2, 0.1, 0.05])[3:-3]
        else:
            amor1 = np.zeros_like(xcor1)
        amor[k] = amor1
        amor2 = np.vstack([amor1 for _ in range(qphi_s.shape[1])])
        cel[k,:,:] = qphi_s[k,:,:]
        cel1 = cel[k,:,:] 
        cel1[:,qidx1] -= amor2 
        
    return sn,cel,amor,xcor1

def separate_cellulose_amporh(dt, qphi_c, cell_peak_pos=[1.55, 1.63], amorph_range=[0.5, 2.0]):
    """ separate anisotropic crystalline cellulose scattering from amorphous scatterting
        used to deal with starch scattering, may be applicable to other components
        source of data is {sn}/qphi/subtracted, generated by sub_qphi_bkg()
        save data to {sn}/qphi/cellulose, and cellulose and amorph under overall/Iq
        qphi_c: assumed q-phi intensity map for crystalline cellulose
            currently this is picked from the data; it could be simulated intensity
        This function converts the angular profile at cell_peak_pos into an angular distribution,
        and extrapolate to the entire q-phi map the crystalline cellulose scattering contribution
        the residue is assumed to be amorphous contrubution
    """
    sns = dt.samples
    pool = mp.Pool(len(sns))
    jobs = []
    ...
    with h5py.File(dt.fn, "r+") as fh5:
        ...
        # prepare for amorphous component separation
        qidx = (xcor>=cell_peak_pos[0])&(xcor<=cell_peak_pos[1])
        idx = ((ycor>=-90)&(ycor<=90))
        y10 = 0 #np.min(y1c)
        prof_phi_c = np.nansum(qphi_c[:,qidx], axis=-1)
        x10 = np.average(ycor[idx], weights=(prof_phi_c-y10)[idx])
        y1c0 = np.interp(ycor, ycor-x10, (prof_phi_c-y10))
        y1c1 = np.interp(ycor[idx], ycor[idx]-x10, (prof_phi_c-y10)[idx])
        y1c2 = (y1c0 + np.flip(y1c0))/2
        pw = int((len(y1c0)-len(y1c1))/2)
        n = pw
        AA = np.vstack([y1c0[pw+i:pw+i+len(y1c1)] for i in range(-n, n+1)]).T
        prof_phi_c = np.nansum(qphi_c[:,qidx], axis=-1)    
    
        lbd1 = 0.
        lbd2 = 0.15
        reg1 = np.identity(AA.shape[-1])
        reg2 = np.zeros_like(reg1)
        for i in range(reg2.shape[0]-1):
            reg2[i,i+1]=-1
        #reg2[i,i] = 0.1 # constant component
        #BB = np.vstack([AA, lbd1*reg1, lbd2*reg2])
        BB = np.vstack([AA, lbd2*reg2])

        # perform separation by multi-processing
        for sn in sns:    
            print(f"begin processing {sn} ...       \r", end="")
            qphi_s = fh5[f"{sn}/qphi/subtracted/d"][...]
            args = (sn, qphi_s, qphi_c, xcor, ycor, BB, cell_peak_pos, amorph_range)
            job = pool.map_async(perform_cellulose_amporh_separation, [args])  
            jobs.append(job)

        pool.close()
        ret = {}
        for job in jobs:
            [sn,cel,amor,xcor1] = job.get()[0]
            ret[sn] = {'cel': cel, 'amor': amor}
            print(f"data received: sn={sn}             \r", end="")
        pool.join()
        print()    

        for sn in sns:
            if "cellulose" in fh5[f"{sn}/qphi"].keys():
                fh5[f"{sn}/qphi/cellulose/d"][...] = ret[sn]['cel']
            else:
                fh5.create_dataset(f"{sn}/qphi/cellulose/d", data=ret[sn]['cel'])
            for k in fh5[f"{sn}/qphi/merged"].attrs.keys():
                fh5[f"{sn}/qphi/cellulose"].attrs[k] = fh5[f"{sn}/qphi/merged"].attrs[k]

    amor_all = np.vstack([ret[sn]['amor'] for sn in sns])
    cell_all = np.vstack([np.nanmean(ret[sn]['cel'], axis=1) for sn in sns])
    dt.add_proc_data("overall", "Iq", "amorph", amor_all)
    dt.add_proc_data("overall", "Iq", "cellulose", cell_all)
    dt.save_data("overall", ["Iq"], ["amorph", "cellulose"])
    dt.set_h5_attr("overall/Iq/amorph", 'qgrid', xcor1)
    dt.set_h5_attr("overall/Iq/cellulose", 'qgrid', xcor)
